...
async def _persist_model_to_db(accuracy: float):
...
async def train(
    ...
    # Bug fix: ไม่ส่ง db session เข้า background task เพราะ session จะปิดหลัง request จบ
    background_tasks.add_task(_persist_model_to_db, accuracy)
